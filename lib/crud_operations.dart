...
//creacion de la base de datos
  ...
  _onCreate(Database db, int version) async {
    await db.execute(
        "CREATE TABLE $TABLE ($Id INTEGER PRIMARY KEY, $NAME TEXT, $APATERNO TEXT, $AMATERNO TEXT, $PHONE TEXT, $EMAIL TEXT, $MATRICULA TEXT)");
  }
  ...
  Future<List<Student>> Students(String mat) async {
    ...
    if(mat==null){
      ...
      List<Map> maps = await dbClient.query(TABLE, columns: [Id, NAME, APATERNO, AMATERNO, PHONE, EMAIL, MATRICULA], where: '$MATRICULA LIKE ?', whereArgs: [mat]);
      if (maps.length > 0) {
        for (int i = 0; i < maps.length; i++) {
          studentss.add(Student.fromMap(maps[i]));
        }
